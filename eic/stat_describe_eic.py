...
def describe_missing(data, var, id_var='noind', time_var='year', time_step='year'):
    ...
    pct_missing = df.groupby([id_var])['missing'].sum() / range_time_var
    ...
    missing_by_anaiss = missing_by_ind.groupby(['Year of Birth'])[[min_time_name, max_time_name, 'Pct. Missing']].mean().astype('f2')
