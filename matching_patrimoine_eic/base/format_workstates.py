...
def define_workstates(career_table, datasets):
    ...
    conditions_by_workstate = {2: (df['unemploy_status'] == 2) & (df['source_salbrut'] == datasets['unemployment']),
                               ...
                               3: ((df['cadre'] != 1) * (df['regimes_by_year'].str.contains('21.0') == False) *
                                  (df['cc'] == 10)),
                               # Private sector, non-executive (Note: cadre could be NaN)
                               4: (df['cadre'] == 1) * (df['cc'] == 10),
                               ...
                               # 6: (df['salbrut_by_year'].str.contains("13")) * (df['cc'] != 10),
                               # Public sector, sedentaire
                               ...
                               12: ((df['cc'] == 21) | ((df['salbrut_by_year'].str.contains('21.0')) * (df['cc'] == 10) *
                                   (df['regimes_by_year'].str.contains('6000') == False)) * (df['cadre'] != 1)), #
    ...
    try:
        ...
        test = pd.concat([condition.astype(float).fillna(0)
